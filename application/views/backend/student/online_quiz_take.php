<?php
$quiz_ends_timestamp = strtotime(date('d-M-Y', $this->db->get_where('online_quiz', array('online_quiz_id' => $online_quiz_id))->row()->quiz_date) . " " . $this->db->get_where('online_quiz', array('online_quiz_id' => $online_quiz_id))->row()->time_end);
...
$questions = $this->db->get_where('question_bank', array('online_quiz_id' => $online_quiz_id))->result_array();
$total_marks = 0;
            ...
            <div class="panel-body table-responsive">
                <div class="row">
                    <div class="col-md-12 text-center">
                        <h3><?php echo $online_quiz_row->title; ?></h3>
                        <h4>
                            <b><?php echo get_phrase('subject'); ?></b>: <?php echo $this->db->get_where('subject', array('subject_id' => $online_quiz_row->subject_id))->row()->name; ?>
                        </h4>
                        <h4>
                            <b><?php echo get_phrase('total_marks'); ?></b>: <?php echo $total_marks; ?>
                        </h4>
                        <h4>
                            <b><?php echo get_phrase('time'); ?></b>: <?php echo ($online_quiz_row->duration / 60) . ' ' . get_phrase('minutes'); ?>
                        </h4>
                        <h4>
                            <b><?php echo get_phrase('quiz_has_to_be_submitted_within') . ': '; ?></b>: <?php echo date('D d-M-Y h:i:s', $quiz_ends_timestamp); ?>
                        </h4>
                        <h5 style="color:green">
                            <b style="color:red"><?php echo get_phrase('instructions'); ?></b>: <?php echo $online_quiz_row->instruction; ?>
                        </h5>
                ...
                <form class="" action="<?php echo site_url('student/submit_online_quiz/' . $online_quiz_id); ?>" method="post" enctype="multipart/form-data" id="answer_script">
                    <?php $count = 1;
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <button type="submit" class="btn btn-success btn-block">
                                <?php echo get_phrase('finish_quiz'); ?>
                            </button>
                        </div>
                    </div>
                </form>
